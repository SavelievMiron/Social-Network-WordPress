...
<tr id="<? the_ID(); ?>" class="meeting" data-row-type="<?= $row_type; ?>" data-status="<?= $status ?>" data-invitor="<?= dao_get_user_display_name( $invitor_id ); ?>" <?= ( $status === 'reschedule' && $curr_user_id == get_post_meta( get_the_ID(), 'reschedule_initiator', true ) ) ? 'data-lp-reschedule-initiator="true"' : 'data-lp-reschedule-initiator="false"'; ?> >
    <td class="title lp-align-left">
        <span class="lp-typo lp-sub lp-grey" title="<? the_title(); ?>"><? the_title(); ?></span>
    </td>

    <td class="datetime lp-align-left">
    ...
    <td class="lp-align-left">
        <span class="lp-typo lp-sub"><? printf( "<a href='%s'>%s</a>", get_author_posts_url( $invitor_id ), $invitor_data->display_name ); ?></span>
    </td>
