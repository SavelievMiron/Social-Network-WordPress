...
<?
    ...
    $customers = array_keys( get_post_meta( $post->ID, 'customers', true ) );
    $curr_user = wp_get_current_user();
?>

<div class="lp-grid lp-item lp-lg-4 lp-md-6 lp-sm-12">
    <article class="lp-card lp-card-offer <?= ( in_array( $curr_user->ID, $customers ) ) ? 'lp-card-mine' : ''; ?> lp-status-<?= $status; ?>" data-lp-card>
        <header class="lp-card__header">
            <div class="lp-card-offer__actions lp-flex lp-align-start lp-justify-between">
                ...
                <? 
                if ( in_array( $curr_user->ID, $customers ) ) : 
                ?>
        ...
        <div class="lp-card__body">
            ...
            <? if ( $card_author === $curr_user->ID && ! empty( $customers ) ) : ?>
            <div class="lp-card-offer__user lp-flex lp-justify-between">
                <span>
                    Заказчики
                </span>
                <span>
                    ...
                    foreach ($customers as $user_id) : 
                        ...
                        $user_name = dao_get_user_display_name( $user_id );

                        $list[] = "<a href='{$user_url}' title='{$user_name}'>{$user_name}</a>";
                    endforeach;
            ...
            <div class="lp-modal" data-lp-modal aria-label="completed card modal" data-lp-card="completed-modal"
                ...
                <form class="lp-modal__wrapper">
                    <div class="lp-paper lp-elevation lp-dense-6 lp-modal__inner" data-lp-modal="content">
                        <? 
                            $card_author_name = dao_get_user_display_name( $card_author );
                            $card_author_link = get_author_posts_url( $card_author );
                        ?>
                        ...
                        <div class="lp-modal__body">
                            <p class="lp-typo lp-body">Вы подтверждаете выполнение исполнителем <a href="<?= $card_author_link ?>"><?= $card_author_name; ?></a> предложения "<?= $post->post_title; ?>"?</p>
                        </div>
                    ...
                    <input type="hidden" name="action" value="dao-complete-offer">
                    <input type="hidden" name="id" value="<?= $post->ID; ?>">
                    <input type="hidden" name="_wpnonce" value="<?= wp_create_nonce('dao-consensus-complete-offer'); ?>">
                </form>
            </div>
            <div class="lp-modal" data-lp-modal aria-label="not completed card modal" data-lp-card="not-completed-modal"
                ...
                <form class="lp-modal__wrapper">
                    <div class="lp-paper lp-elevation lp-dense-6 lp-modal__inner" data-lp-modal="content">
                        ...
                        <div class="lp-modal__body">
                            <p class="lp-typo lp-body">Вы подтверждаете не выполнение исполнителем <a href="<?= $card_author_link ?>"><?= $card_author_name; ?></a> предложения "<?= $post->post_title; ?>"?</p>
                        </div>
                    ...
                    <input type="hidden" name="action" value="dao-not-complete-offer">
                    <input type="hidden" name="id" value="<?= $post->ID; ?>">
                    <input type="hidden" name="_wpnonce" value="<?= wp_create_nonce('dao-consensus-not-complete-offer'); ?>">
                </form>
            </div>
        <? endif; ?>
