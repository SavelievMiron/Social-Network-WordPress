...
<?
    ...
    $card_author = (int) get_post_field( 'post_author', get_the_ID() );
    $performer = (int) $post->performer;
    ...
    <article class="lp-card lp-card-offer <?= ($card_author === $curr_user->ID) ? 'lp-card-mine' : ''; ?> lp-status-<?= $status; ?>" data-lp-card>
        <header class="lp-card__header">
            <div class="lp-card-offer__actions lp-flex lp-align-start lp-justify-between">
                ...
                if ( $card_author === (int) get_current_user_id() ) : 
                ?>
        ...
        <div class="lp-card__body">
            ...
            <div class="lp-card-offer__user lp-flex lp-justify-between">
                ...
                <span
                    class="lp-typo lp-sub"><b><?= esc_html( get_post_meta( get_the_ID(), 'total_price', true ) ); ?></b></span>
            </div>
            <hr class="lp-divider" />
                ...
                <form class="lp-modal__wrapper">
                    <div class="lp-paper lp-elevation lp-dense-6 lp-modal__inner" data-lp-modal="content">
                        ...
                        <div class="lp-modal__body">
                            <p class="lp-typo lp-body">Вы подтверждаете выполнение исполнителем <a href="<?= $performer_link ?>"><?= $performer_name; ?></a> <?= ( $post->post_type === 'demands' ) ? 'cпроса' : 'предложения'; ?> "<?= $post->post_title; ?>"?</p>
                        </div>
                    ...
                    <input type="hidden" name="action" value="<?= ($post->post_type === 'demands') ? 'dao-complete-demand' : 'dao-complete-offer'; ?>">
                    <input type="hidden" name="id" value="<?= $post->ID; ?>">
                    <input type="hidden" name="_wpnonce" value="<?= ($post->post_type === 'demands') ? wp_create_nonce('dao-consensus-complete-demand') : wp_create_nonce('dao-consensus-complete-offer'); ?>">
                </form>
            </div>
            <div class="lp-modal" data-lp-modal aria-label="not completed card modal" data-lp-card="not-completed-modal"
                ...
                <form class="lp-modal__wrapper">
                    <div class="lp-paper lp-elevation lp-dense-6 lp-modal__inner" data-lp-modal="content">
                        ...
                        <div class="lp-modal__body">
                            <p class="lp-typo lp-body">Вы подтверждаете не выполнение исполнителем <a href="<?= $performer_link ?>"><?= $performer_name; ?></a> <?= ( $post->post_type === 'demands' ) ? 'cпроса' : 'предложения'; ?> "<?= $post->post_title; ?>"?</p>
                        </div>
                    ...
                    <input type="hidden" name="action" value="<?= ($post->post_type === 'demands') ? 'dao-not-complete-demand' : 'dao-not-complete-offer'; ?>">
                    <input type="hidden" name="id" value="<?= $post->ID; ?>">
                    <input type="hidden" name="_wpnonce" value="<?= ($post->post_type === 'demands') ? wp_create_nonce('dao-consensus-not-complete-demand') : wp_create_nonce('dao-consensus-not-complete-offer'); ?>">
                </form>
            </div>
        <? endif; ?>
